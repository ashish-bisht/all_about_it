    ...
    questions: [
        {
            ...
            learn: {
                ...
                revision: {
                    oneLiner: "One-pass HashMap: for each num, check if (target - num) is already seen; if yes return indices, else store num->index.",
                    bullets: [
                        "Rule: need = target - num → if need in seen → done",
                        "Order: check first, then store (prevents self-usage)",
                        "Time O(N), Space O(N)"
                    ],
                    miniExample: "nums=[2,7,11,15], target=9 → seen={2:0} → at index=1 num=7 need=2 → found → [0,1]"
                },
                ...
                visual: `<div style="font-family:ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; font-size:0.85rem; line-height:1.55;">
  <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:10px;">
    <strong style="color:#6366f1; font-size:0.95rem;">Two Sum — Complement Mirror</strong>
    <span style="background:rgba(99,102,241,0.12); border:1px solid rgba(99,102,241,0.35); color:#a5b4fc; padding:4px 10px; border-radius:999px; font-weight:700;">target = 9</span>
  </div>

  <div style="background:#1e293b; padding:14px; border-radius:12px; border:1px solid rgba(255,255,255,0.06);">
    <div style="color:#94a3b8; font-size:0.78rem; margin-bottom:10px;">nums = [2, 7, 11, 15]</div>

    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
      <!-- PASS (timeline) -->
      <div style="background:rgba(15,23,42,0.55); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:12px;">
        <div style="color:#e2e8f0; font-weight:800; margin-bottom:8px;">One-pass scan</div>

        <div style="display:flex; flex-direction:column; gap:8px;">
          <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
            <span style="color:#94a3b8;">index=0</span>
            <span style="padding:5px 12px; background:rgba(251,191,36,0.18); border:1px solid rgba(251,191,36,0.45); border-radius:10px; color:#fbbf24; font-weight:900;">num=2</span>
            <span style="color:#94a3b8;">need <span style="color:#fbbf24; font-weight:800;">7</span></span>
          </div>

          <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
            <span style="color:#94a3b8;">index=1</span>
            <span style="padding:5px 12px; background:rgba(74,222,128,0.16); border:1px solid rgba(74,222,128,0.45); border-radius:10px; color:#4ade80; font-weight:900;">num=7</span>
            <span style="color:#94a3b8;">need <span style="color:#4ade80; font-weight:800;">2</span></span>
          </div>
        </div>
      </div>
      ...
      <!-- MAP (memory) -->
      <div style="background:rgba(15,23,42,0.55); border:1px solid rgba(255,255,255,0.06); border-radius:10px; padding:12px;">
        <div style="color:#e2e8f0; font-weight:800; margin-bottom:8px;">HashMap = memory</div>

        <div style="display:flex; flex-direction:column; gap:8px;">
          <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
            <span style="color:#94a3b8;">after index=0</span>
            <span style="color:#a5b4fc;">seen = { <span style="color:#fbbf24; font-weight:900;">2:0</span> }</span>
          </div>
          <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
            <span style="color:#94a3b8;">at index=1</span>
            <span style="color:#4ade80; font-weight:900;">need(2) ∈ seen ✅</span>
          </div>
        </div>
      </div>
    </div>

    <div style="margin-top:12px; padding:10px 12px; background:rgba(74,222,128,0.10); border-radius:10px; border:1px solid rgba(74,222,128,0.25);">
      <div style="color:#4ade80; font-weight:900;">Answer: indices [0, 1]</div>
      <div style="color:#94a3b8; font-size:0.78rem;">Because nums[0]=2 and nums[1]=7 → 2 + 7 = 9</div>
    </div>
  </div>

  <div style="margin-top:10px; background:#0f172a; padding:10px; border-radius:10px; border:1px solid rgba(255,255,255,0.06);">
    <div style="color:#fbbf24; font-weight:900;">Rule:</div>
    ...
    for index, num in enumerate(nums):
        ...
        # store AFTER checking (prevents using the same element twice)
        seen[num] = index
    ...
    return []`
        ...
        },
        {
            ...
            learn: {
                ...
                revision: {
                    oneLiner: "Sort → fix an anchor → run two pointers (L/R). If sum==0, record and skip duplicates.",
                    bullets: [
                        "Anchor loop: skip duplicates (if nums[index]==nums[index-1])",
                        "When found: left++, right-- then skip dup left/right",
                        "Time O(N^2), Space O(1) (output ignored)"
                    ],
                    miniExample: "nums=[-1,0,1,2,-1,-4] → sort=[-4,-1,-1,0,1,2] → anchor=-1 → pairs: (-1,2) and (0,1)"
                },
  ...
  <div style="margin-top:10px; background:var(--bg-surface); border:1px solid var(--border); border-radius:12px; padding:10px 12px;">
    ...
    <div style="color:var(--text-muted); font-size:0.78rem;">
      ...
      Found triplet ke baad: L/R duplicates while-skip.
                ...
                whyItWorks: "<strong>Why this works:</strong><br>After sorting, if <code>total</code> is too small we must increase it → move <code>L</code> right. If <code>total</code> is too big we must decrease it → move <code>R</code> left. This monotonic move guarantees we explore all unique pairs for each anchor.",
                dryRun: [
    ...
    for index in range(len(nums) - 2):
        ...
        while left < right:
            ...
            if total == 0:
                ans.append([anchor, nums[left], nums[right]])
                left += 1
                right -= 1
    ...
    return max_so_far`
        ...
        },
        {
            ...
            learn: {
                ...
                metrics: { time: "O(N)", space: "O(1)" },
                revision: {
                    oneLiner: "Two-pass prefix/suffix: first store left products in res, then multiply by right products while scanning from right.",
                    bullets: [
                        "Pass-1 (L→R): res[index] = product of all left",
                        "Pass-2 (R→L): res[index] *= product of all right",
                        "No division; handles zeros automatically",
                        "Time O(N), Extra space O(1) (output excluded)"
                    ],
                    miniExample: "nums=[1,2,3,4] → left pass res=[1,1,2,6] → right pass multiply → [24,12,8,6]"
                },
...
<div style="font-family:var(--font-mono); font-size:0.85rem; line-height:1.6;">
  ...
  <div style="margin-top:10px; background:var(--bg-surface); border:1px solid var(--border); border-radius:12px; padding:10px 12px;">
    ...
    <div style="color:var(--text-muted); font-size:0.78rem;">res[i] = (product of left) × (product of right)</div>
                ...
                codeTitle: "Python Solution (Two Pass)",
        ...
        },
        {
            ...
            learn: {
                quickAlgo: [
                    ...
                    "trapped_water = 0",
                    "while left < right:",
                    "    if height[left] < height[right]:               # 🔄 Left side smaller",
                    "        if max_left_height > height[left]:         # Valley → water",
                    "            trapped_water += max_left_height - height[left]",
                    "        else:",
                    "            max_left_height = height[left]         # New wall",
                    "        left += 1",
                    "    else:                                         # Right side smaller",
                    "        if max_right_height > height[right]:      # Valley → water",
                    "            trapped_water += max_right_height - height[right]",
                    "        else:",
                    "            max_right_height = height[right]      # New wall",
                    "        right -= 1",
                    "return trapped_water"
                ],
                metrics: { time: "O(N)", space: "O(1)" },
                revision: {
                    oneLiner: "Two pointers from both ends. Always move the smaller side and use its max-wall to add trapped water.",
                    bullets: [
                        "If height[left] < height[right] → process left (right side has a taller wall somewhere)",
                        "Water at left = max_left_height - height[left] (if positive)",
                        "Else update max_left_height; same logic for right",
                        "Time O(N), extra space O(1)"
                    ],
                    miniExample: "height=[0,1,0,2] → trapped=1"
                },
    ...
    <div style="display:grid; grid-template-columns:1fr; gap:8px;">
      ...
      <div style="background:color-mix(in srgb, var(--success) 10%, transparent); border:1px solid color-mix(in srgb, var(--success) 30%, transparent); border-radius:12px; padding:10px 12px;">
        ...
        <div style="color:var(--text-secondary);">[0,1,0,2] me beech ka 0, left max=1 aur right me wall=2 → water = 1-0 = 1</div>
                ...
                codeTitle: "Python Solution (Two Pointers)",
                code: `def trapping_water(nums):
...
window.currentTopicData = topic_arrays;
